...
# Import LLM tools
from tools_functions import get_tools_info

# Import embedding functions
from workload_embedding import get_embedding_log

# Import utility functions
from workload_game_cache import get_cache_info
...
def build_cache_log(client, session_id, message_id):
    ...
    try:
        ...
        cache_response = create_response(
            2, cache_text, session_id, f"{message_id}_cache"
        )
        ...
        cache_response = create_response(
            2, error_text, session_id, f"{message_id}_cache"
        )
        send_response(client, cache_response, session_id, 2, f"{message_id}_cache")
...
def build_tools_log(client, session_id, message_id):
    ...
    try:
        ...
        tools_response = create_response(
            3, tools_text, session_id, f"{message_id}_tools"
        )
        ...
        tools_response = create_response(
            3, error_text, session_id, f"{message_id}_tools"
        )
        send_response(client, tools_response, session_id, 3, f"{message_id}_tools")
