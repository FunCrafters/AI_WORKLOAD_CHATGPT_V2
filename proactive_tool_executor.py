...
from screen_tool_config import build_prompt_injection, get_screen_tool_rules
            ...
            if "data_tools" in rules:
                ...
                for data_tool in rules["data_tools"]:
                    ...
                    if json_field and json_field in data_fields:
                        ...
                        logger.info(
                            f"✅ Added data tool: {tool_config['tool']} with {json_field}={data_fields[json_field]}"
                        )
                    else:
                        logger.warning(
                            f"⚠️ Data tool {data_tool['tool']} skipped - field '{json_field}' not found"
                        )
...
def execute_proactive_tools(
    ...
    for tool_config in tools_to_execute:
        ...
        try:
            ...
            logger.info(
                f"🔧 Executing proactive tool: {tool_name} with params: {parameters}"
            )
    ...
    logger.info(
        f"✅ Proactive tool execution completed. Generated {len(tool_results)} messages"
    )
    return tool_results
...
def get_proactive_tool_messages(
    ...
    logger.info(
        f"✅ Proactive analysis completed. Tools: {len(tool_messages)} messages, Prompt: {len(prompt_injection)} chars"
    )
