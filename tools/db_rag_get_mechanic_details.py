...
# Logger
logger = logging.getLogger("DB Mechanics Details")
...
def db_rag_get_mechanic_details(query: str) -> str:
    """
    ...
    try:
        ...
        # Search for similarity results (MECHANICS chunk_section, not QA)
        similarity_query = """
        ...
        # Search for QA results (MECHANICS chunk_section and QA)
        qa_query = """
            SELECT chunk_text, metadata, 1 - (embedding <=> %s::vector) as similarity
            FROM rag_vectors 
            WHERE metadata->>'chunk_section' = 'MECHANICS'
        ...
        logger.error(f"Error in db_rag_get_mechanic_details: {str(e)}")
        return json.dumps({
            "status": "error",
            "message": f"Database error while searching for mechanics '{query}'",
            "search_query": query,
            "category": "mechanics",
            "content": {
                "similarity_results": "",
