...
def db_find_strongest_champions(
    limit: int = 10,
    ...
    try:
        ...
        for i, champion in enumerate(champions, 1):
            champion_list.append(
                f"{i}. **{champion['champion_name']}** - Power: {champion['total_power']} ({champion.get('rarity', 'N/A')} {champion.get('affinity', 'N/A')} {champion.get('class', 'N/A')})"
            )
