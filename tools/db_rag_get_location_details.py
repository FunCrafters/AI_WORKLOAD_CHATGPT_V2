...
# Logger
logger = logging.getLogger("DB Locations")
...
def db_rag_get_location_details(query: str) -> str:
    """
    ...
    try:
        ...
        # Search for similarity results (LOCATIONS chunk_section, not QA)
        similarity_query = """
        ...
        cursor.execute(similarity_query, (query_embedding, query_embedding, SIMILARITY_THRESHOLD, SIMILARITY_LIMIT))
        similarity_rows = cursor.fetchall()
        
        # Search for QA results (LOCATIONS chunk_section and QA)
        qa_query = """
            SELECT chunk_text, metadata, 1 - (embedding <=> %s::vector) as similarity
            FROM rag_vectors 
            WHERE metadata->>'chunk_section' = 'LOCATIONS'
        ...
        cursor.execute(qa_query, (query_embedding, query_embedding, SIMILARITY_THRESHOLD, SIMILARITY_LIMIT))
        ...
        if not has_similarity and not has_qa:
            return json.dumps({
                "status": "error",
                "message": f"No location information found for query '{query}'",
                "search_query": query,
                "category": "locations",
                "content": {
                    "similarity_results": "",
        ...
        return json.dumps({
            "status": "success",
            "message": f"Found location information for '{query}'",
            "search_query": query,
            "category": "locations",
            "content": {
        ...
        logger.error(f"Error in db_rag_get_location_details: {str(e)}")
        return json.dumps({
            "status": "error",
            "message": f"Database error while searching for location '{query}'",
            "search_query": query,
            "category": "locations",
            "content": {
                "similarity_results": "",
